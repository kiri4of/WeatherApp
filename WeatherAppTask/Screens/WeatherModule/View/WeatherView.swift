...
class WeatherView: UIView {
    ...
    convenience init(weatherData: CurrentWeatherAPIEnum.CurrentWeatherUIModel) {
        self.init()
    ...
    func saveWeatherData(weatherData: CurrentWeatherAPIEnum.CurrentWeatherUIModel) {
        let encoder = JSONEncoder()
        if let encodedData = try? encoder.encode(weatherData) {
            UserDefaults.standard.set(encodedData, forKey: "weatherDataKey")
        }
    }
...
extension WeatherView {
    func setupViews() {
        ...
        temperatureLabel.snp.makeConstraints { make in
            make.centerX.equalToSuperview()
            make.centerY.equalToSuperview().offset(-100) // Немного выше центра
        }
...
//StackView для humidity и pressure и сделать так чтобы текст был видеен полностью + сделать какой-то интерефейс и добавить сверху название города и мб кнопку refresh а так же сделать неактивную кнопку button пока textField пустой.
